 ...
 * совсем не имеют логики (исключением должен являться только наследуемый метод {@link #destruct()}. Это правило
 * желательно соблюдать, однако его несоблюдение не повлияет на работу игрового движка). Все данные каждого условного
 * игрового объекта представляют собой исключительно набор компонентов. Сами же игровые объекты - это множество
 * компонентов логически связанных между собой через одну и туже сущность (см. {@link Entity}). Каждый компонент
 * может быть связан только с одной сущностью. Сущность, с которой связан компонент, называется сущностью-владельцем
 * данного компонента. Сущность-владелец может быть назначена компоненту только один раз при его создании.
 */
public abstract class Component {
    ...
    * EntityComponentManager, данный метод полагается на то, что removed присутсвует в списке.
    ...
    /**
     * Вызывается в момент удаления сущности, которой принадлежит данный компонент. Данный метод может быть
     * использован, например, чтобы компонент мог удалить себя из дополинтельной структуры данных используемой
