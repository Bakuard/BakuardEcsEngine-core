 ...
 * применены к {@link EntityComponentManager} в порядке их добавления в буфер.
 * Используйте объекты этого класса, если выполняется хотя бы одно из перечисленных ниже условий:
 * 1. Необходимо создавать, удалять или изменять состав компонентов сущностей во время итерации по ним.
 * 2. Необходимо отложенно выполнить ряд последовательных операций.
...
public class EntityCommandBuffer {
     ...
     * (см. {@link Component#Component(Entity, int)}) в очередь отложенных операций. Сущность может быть связана с
     * несколькими компонентами одного и того же типа.
