 ...
 * При попытке работать с одними и теми же сущностями и компонентами через разные менеджеры сущностей может привести
...
public final class EntityComponentManager {
     ...
     * Как и все итераторы Java, данный итератор придерживается политики fail-fast.
     ...
     * императивный стиль перебора сущностей, используйте метод {@link #getEntities(EntitiesFilter)}. Данный метод
     * придерживается политики fail-fast.
     * @param FILTER объект задающий критерий отбора сущностей.
    ...
    /**
     * Возвращает true, если указанная сущность связана хотя бы с одним компонентом идентификатор типа которого
     * равен componentTypeID, иначе возвращает false.
     * @param owner сущность для которой выполняется проверка.
     * @param componentTypeID идентификатор типа компонента.
     * @return true, если указанная сущность связана хотя бы с одним компонентом идентификатор типа которого
     *         равен componentTypeID, иначе возвращает false.
     */
     ...
     * будет возвращен. Если сущность не связана ни с одним компонентом указанного типа - возвращает null.
     * @param owner сущность для которой выполняется проверка.
     * @param componentTypeID идентификатор типа компонентов.
     * @return компонент ассоциированый с указанной сущностью.
     */
    ...
    /**
     * Возвращает компонент связанный с указанной сущностью и имеющий указаную метку. Если сущность не связана
     * ни с одним компонентом указанного типа и имеющим указаную метку - возвращает null. Если сущность связана
     * сразу с несколькими компонетами указанного типа и имеющими указаную метку - возвращает один из них, при
     * этом метод не дает гарантий касательно того, кокой именно компонент будет возвращен.
     * @param owner сущность для которой выполняется проверка.
     * @param componentTypeID идентификатор типа компонентов.
     * @param label строковая метка по которой ищется компонент асоциированый с указываемой сущностью.
     * @return компонент ассоциированый с указанной сущностью и имеющий указаную метку.
     */
     ...
     * Сущность может быть связана с несколькими компонентами одного и того же типа. Информация о связи между
     * сущностю-владельцем и компонентом будет сохранена в данном менеджере сущностей и компонентов.
     ...
     * (см. {@link Component#Component(Entity, int)}). Сущность может быть связана с несколькими компонентами одного
     * и того же типа. Данный метод требует, чтобы все компоненты связывались с одной сущностью. Информация о связи
     * между сущностю-владельцем и компонентами будет сохранена в данном менеджере сущностей и компонентов. В случае,
     * если выполнение метода завершается исключением - ни один компонент не будет связан с сущностью.
     * @param components связываемые компоненты.
     * @throws IllegalArgumentException генериурется по одной из следущих причин:
     *                                  1. Передаваемый массив компонентов - пуст.
     *                                  2. Передаваемые компоненты связываются с разными сущностями.
     *                                  3. Сущность, с которой собираются связаться компоненты - удалена.
     ...
     *                                  1. Передаваемый массив компонентов - пуст.
     *                                  2. Передаваемые компоненты связываются с разными сущностями.
     *                                  3. Сущность, от которой собираются отвязаться компоненты - удалена.
     *                                  4. Один из передаваемых компонентов уже связан с сущностью.
     */
        ...
        while(!buffer.isEmpty()) {
            try {
                ...
                try {
                    ...
                    throw new BatchException(exceptionFromHandler);
                }
            }
        }
    }
