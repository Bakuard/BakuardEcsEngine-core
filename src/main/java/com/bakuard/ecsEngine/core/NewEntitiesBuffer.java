 ...
 * Иначе говоря, используйте объекты этого класса, если выполняются хотябы одно из ниже перечисленных условий:
 * 1. Асинхронно создать большое кол-во сущностей и асинхронно связать их с компонентами.
 * 2. Указанные в первом пункте действия должны быть выполнены в отдельном потоке.
...
public final class NewEntitiesBuffer {
     ...
     * сущностю-владельцем и компонентом будет сохранена только в данном буфере.
     ...
     * (см. {@link Component#Component(Entity, int)}). Сущность может быть связана с несколькими компонентами
     * одного и того же типа. Данный метод требует,  чтобы все компоненты связывались с одной сущностью. Информация
     * о связи между сущностю-владельцем и компонентами будет сохранена только в данном буфере. В случае, если
    ...
    /**
     * Возвращает сущность созданую через данный буфер, хранящуюся в нем и имеющую указанный personalID
     ...
     * В буфере данног типа могут находится только сущности созданные через него.
